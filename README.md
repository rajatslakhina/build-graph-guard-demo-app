...
2. **The look-alike re-point.** `example-org` becomes `example-0rg`, the pin moves from a version to a branch, the deployment floor drops to 15.0, and the app target starts compiling a file two directories above the repository. Every version number on screen stays plausible. **Blocked.**
...
└── Demo (app target)
    ...
    │   requirement: upToNextMajorVersion, minimumVersion 1.1.0
...
- The library repo itself: `swift build -Xswiftc -warnings-as-errors` clean from a wiped `.build`, 111 tests passing, and green CI on Linux and macOS.
